<template>
  ...
  <div class="card-header text-center">
     <span v-if="!$route.params.id">Add</span> <span v-if="$route.params.id > 0">Edit</span> Department
  </div>
...
export default {
  ...
  onAddEdit(){
    if(this.$route.params.id){
        ...
        this.addEditDepartment({id: 0, data: this.myObject});
    }
  }
  },
  created(){
   ...
   // fetchUser #editMode
    ...
    if(this.$route.params.id > 0){
       // edit
        if(this.userPermissionsGlobal && !this.userPermissionsGlobal.includes('edit_departments')){
          this.$router.push('/');
        }
    }else{
      // add
      if(this.userPermissionsGlobal && !this.userPermissionsGlobal.includes('add_departments')){
         this.$router.push('/');
        }
    }
    // permissions end
